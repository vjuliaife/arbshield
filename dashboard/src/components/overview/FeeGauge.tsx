...
function feeColor(percent: number): string {
  if (percent < 0.15) return '#22C55E'
  if (percent < 0.4)  return '#EAB308'
  if (percent < 0.7)  return '#F97316'
  return '#EF4444'
}
      ...
      <div className="relative">
        <svg width="200" height="160" viewBox="0 0 200 180">
          <circle
            ...
            fill="none" stroke="#1B2234" strokeWidth="14" strokeLinecap="round"
          ...
          <text x={CX} y={CY - 8} textAnchor="middle" fill="white" fontSize="26" fontWeight="bold" fontFamily="JetBrains Mono">
            {formatFee(fee)}
          </text>
          <text x={CX} y={CY + 14} textAnchor="middle" fill="#98A1C0" fontSize="10" fontFamily="JetBrains Mono">
            {fee} bps
          </text>
        </svg>
        {elevated && (
